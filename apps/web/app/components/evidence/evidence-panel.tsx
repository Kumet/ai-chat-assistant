...
type RendererAwareCore = CytoscapeCore & {
	renderer?: () => CytoscapeRenderer | null;
};
...
function getRenderer(core: CytoscapeCore): CytoscapeRenderer | null {
	const rendererFn = (core as RendererAwareCore).renderer;
	if (typeof rendererFn !== "function") {
		return null;
	}
	try {
		return rendererFn.call(core) ?? null;
	} catch {
		return null;
	}
}
...
export function EvidencePanel({ analysis }: EvidencePanelProps) {
	...
	useEffect(() => {
		const cy = cyRef.current;
		if (!cy || cy.destroyed?.()) return;
		if (!getRenderer(cy)) return;
		if (symbolCount === 0) return;
		const shouldRunLayout = symbolCount > 1 || edgeCount > 0;
